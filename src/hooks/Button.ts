...
export function Check(data: videoType | undefined) {
  let like = data?.Like.filter((data) => data.channel == token.channel.message )
  let disLike = data?.DisLike.filter((data) => data.channel == token.channel.message)
  let subscribeCheck = data?.subscribe.filter((data) => data.subScribeChannel == token.channel.message)
  if (like?.length == 1) {
